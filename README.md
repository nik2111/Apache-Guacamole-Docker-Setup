Guacamole actually consists of three parts the guacamole frontend, the backend and the database. For more in depth understanding please refer the [https://guacamole.apache.org/doc/gug/guacamole-architecture.html manual]
...
check if the container is running (you can also use portainer)
 sudo docker ps

deploy the postgresq container. For a quick postgress deployment tutorial see [https://dev.to/shree_j/how-to-install-and-run-psql-using-docker-41j2 link]
...
Create the database that guacamole will use. The name guacamole_db is required for guacamole to work. Check [https://www.postgresql.org/docs/9.1/app-createdb.html this link] if you have trouble with psql commands
...
run the [https://stackoverflow.com/questions/8208181/create-postgres-database-using-batch-file-with-template-encoding-owner-and database initialization script]
